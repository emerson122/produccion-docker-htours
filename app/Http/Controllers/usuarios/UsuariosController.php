...
class UsuariosController extends Controller
{
    ...
    public function insertar( Request $req)
    {
        ...
        if ($update == '1') {
            try {
        $usr = http::withToken(Cache::get('token'))->post($this->url.'/usuario/insertar/interno2',[

                    "USR" => $req->USUARIO,
                    "NOM_USUARIO" => $req->NOMBRE_USUARIO,
                    "PASS" => md5($req->passw),
                    "CORREO" =>  $req->CORREO
        ]);
        ...
        $correodup = 0;
        $userdup = 0;
        $correodup = strrpos($usr, "CORREO DUPLICADO");
        $userdup = strrpos($usr, "USUARIO DUPLICADO");
        if ($correodup > 0) {
            # code...
            Session::flash('correodup','1');
            return back();
        }
        if ($userdup > 0) {
            # code...
            Session::flash('userdup','1');
            return back();
        }
        ...
        try {
            $bitacora = Http::withToken(Cache::get('token'))->post($this->url . '/seguridad/bitacora/insertar', [
                ...
                "DES" => Cache::get('user') . ' INSERTO UN USUARIO A '. $req->USUARIO,
                "OBJETO" => 'USUARIOS'
            ]);
        } catch (\Throwable $th) {
            //throw $th;
